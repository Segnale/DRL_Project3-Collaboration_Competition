...
if __name__ == "__main__":
    ...
    for ep_i in range(episodes):
        ...
        for step_i in range(steps):
            ...
            action[0] = agent.act(state[0], 0)
            action[1] = agent.act(state[1], 1)
            ...
            # step first agent 
            agent.step(state[0], action[0], reward[0], next_state[0], done[0])#, 0)
            # step second agent
            agent.step(state[1], action[1], reward[1], next_state[1], done[1])#, 1)
